...
ChatGPT<WiFiClientSecure> chat_gpt(&client, "v1", "OPEN-API-KEY");   // Please fill in your OpenAI key


// Please change to yourtwork
...
bool uploadFile(){
  ...
  HTTPClient client;
  client.begin("http:// 192.168.50.121:8888/uploadAudio");
...
void record(){
  if(recordTask){
    ...
    lv_label_set_text(ui_question, "Listening ...");
    lv_timer_handler();
...
void chatgpt(){
  if(chatgptTask){
    ...
    lv_timer_handler();
    String result;
    if (chat_gpt.simple_message("gpt-3.5-turbo-0301", "assistant", response, result)) {
      Serial.println("===OK===");
